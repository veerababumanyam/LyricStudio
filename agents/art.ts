...
export const runArtAgent = async (
  ...
  try {
    ...
    const response = await retryWithBackoff(async () => await ai.models.generateImages({
    ...
    // For Art, we often fail silently or show a toast, throwing wraps it for the UI to handle
